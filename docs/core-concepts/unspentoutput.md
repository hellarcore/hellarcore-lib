...
`hellarcore.Transaction.UnspentOutput` is a class with stateless instances that provides information about an unspent output:

- Transaction ID and output index
- The "scriptPubKey", the script included in the output
- Amount of hellars associated
...
- `amount`: expected value in BTC. If the `hellars` alias is used, make sure to use hellars instead of BTC.
