# Contributing to hellarcore-lib

We're working hard to make _hellarcore-lib_ the most powerful JavaScript library for working with Hellar. Our goal is to have _hellarcore-lib_ be a library that can be used by anyone interested in Hellar, and to level expertise differences with great design and documentation.
...
These are some global design goals in hellarcore-lib that any change must adhere.
...
#### G3 - Internal Amounts Must be Integers Representing hellars

Avoid representation errors by always dealing with hellars. For conversion for frontends, use the `Unit` class.
...
When possible, hellarcore-lib objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of hellarcore-lib methods. For example, calling `JSON.stringify` on an hellarcore-lib object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'hellarcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the hellarcore object.

DO:

```javascript
var hellarcore = require('../');
var PublicKey = hellarcore.PublicKey;
```
...
```sh
git remote add hellarcore git@github.com:hellarcore/hellarcore-lib.git
git pull --rebase hellarcore master
```
...
Finally go to [github.com/hellarcore/hellarcore-lib](https://github.com/hellarcore/hellarcore-lib) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository. We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of hellarcore-lib.

If you have any questions feel free to post them to
[github.com/hellarcore/hellarcore-lib/issues](https://github.com/hellarcore/hellarcore-lib/issues).
