...
describe('Transaction', function () {
  ...
  it('can serialize to a plain javascript object', function () {
    ...
    object.inputs[0].output.hellars.should.equal(testAmount);
    ...
    object.outputs[0].hellars.should.equal(testAmount - 10000);
  });
  ...
  it('toObject/fromObject with signatures and custom fee', function () {
    var tx = new Transaction()
      .from(simpleUtxoWith100000hellars)
      .to([{ address: toAddress, hellars: 50000 }])
      .fee(15000)
  ...
  it('fromObject with pay-to-public-key previous outputs', function () {
    ...
    tx.inputs[0].output.hellars.should.equal(5000000000);
  ...
  var simpleUtxoWith100000hellars = {
    address: fromAddress,
  ...
  var simpleUtxoWith1000000hellars = {
    address: fromAddress,
    txId: 'a477af6b2667c29670467e4e0728b685ee07b240235771862318e29ddbe58458',
    outputIndex: 0,
    script: Script.buildPublicKeyHashOut(fromAddress).toString(),
  ...
  var anyoneCanSpendUTXO = JSON.parse(
    JSON.stringify(simpleUtxoWith100000hellars)
  );
  ...
  describe('change address', function () {
    it('can calculate simply the output amount', function () {
      ...
      transaction.outputs[1].hellars.should.equal(49000);
    ...
    it('accepts a P2SH address for change', function () {
      var transaction = new Transaction()
        .from(simpleUtxoWith1000000hellars)
        .to(toAddress, 500000)
    ...
    it('can recalculate the change amount', function () {
      ...
      transaction.getChangeOutput().hellars.should.equal(50000);

      transaction = transaction.to(toAddress, 20000).sign(privateKey);

      transaction.outputs.length.should.equal(3);
      transaction.outputs[2].hellars.should.equal(30000);
    ...
    it('fee can be set up manually', function () {
      var transaction = new Transaction()
        ...
        .sign(privateKey);
      transaction.outputs.length.should.equal(2);
      transaction.outputs[1].hellars.should.equal(10000);
    });
    it('fee per kb can be set up manually', function () {
      var inputs = _.map(_.range(10), function (i) {
        var utxo = _.clone(simpleUtxoWith100000hellars);
        utxo.outputIndex = i;
        return utxo;
      });
      var transaction = new Transaction()
      ...
      transaction.outputs.length.should.equal(2);
      transaction.outputs[1].hellars.should.equal(34000);
    });
    it('if hellars are invalid', function () {
      var transaction = new Transaction()
        .from(simpleUtxoWith100000hellars)
        .to(toAddress, 99999)
        .change(changeAddress)
        .sign(privateKey);
      transaction.outputs[0]._hellars = 100;
      transaction.outputs[0]._hellarsBN = new BN(101, 10);
      expect(function () {
        return transaction.serialize();
      }).to.throw(errors.Transaction.Invalidhellars);
    });
  ...
  describe('#verify', function () {
    it('not if _hellars and _hellarsBN have different values', function () {
      var tx = new Transaction()
        .from({
          txId: testPrevTx,
          outputIndex: 0,
      ...
      tx.outputs[0]._hellars = 100;
      tx.outputs[0]._hellarsBN = new BN('fffffffffffffff', 16);
      var verify = tx.verify();
      verify.should.equal('transaction txout 0 hellars is invalid');
    });

    it('not if _hellars is negative', function () {
      var tx = new Transaction()
        .from({
          txId: testPrevTx,
          outputIndex: 0,
      ...
      tx.outputs[0]._hellars = -100;
      tx.outputs[0]._hellarsBN = new BN(-100, 10);
      var verify = tx.verify();
      verify.should.equal('transaction txout 0 hellars is invalid');
    });
  ...
  describe('removeInput and removeOutput', function () {
    it('can remove an input by index', function () {
      ...
      transaction.inputAmount.should.equal(simpleUtxoWith1BTC.hellars);
    ...
    it('an output can be removed by index', function () {
      var transaction = new Transaction().to([
        { address: toAddress, hellars: 40000000 },
        { address: toAddress, hellars: 40000000 },
      ]);
      transaction.outputs.length.should.equal(2);
  ...
  describe('output ordering', function () {
    ...
    beforeEach(function () {
      transaction = new Transaction()
        .from(simpleUtxoWith1BTC)
        .to([
          { address: toAddress, hellars: tenth },
          { address: toAddress, hellars: fourth },
        ])
        .to(toAddress, half)
        .change(changeAddress);
  ...
  describe('clearOutputs', function () {
    it('removes all outputs and maintains the transaction in order', function () {
      var tx = new Transaction()
        ...
        .to([
          { address: toAddress, hellars: fourth },
          { address: toAddress, hellars: half },
        ])
        .change(changeAddress);
      tx.clearOutputs();
      ...
      tx.outputs[0].hellars.should.equal(10000000);
      tx.outputs[0].script.toAddress().toString().should.equal(toAddress);
      tx.outputs[1].hellars.should.equal(89999000);
  ...
  describe('BIP69 Sorting', function () {
    ...
    it('sorts outputs correctly', function () {
      ...
      tx.sort();
      tx.outputs[0].hellars.should.equal(1);
      tx.outputs[1].hellars.should.equal(2);
      tx.outputs[2].hellars.should.equal(2);
  ...
  describe('addBurnOutput', function () {
    it('Should add burn output with a public key hash', function () {
      ...
      expect(transaction.outputs[0].hellars).to.be.equal(10000);
...
var unsupportedTxObj =
  '{"version":1,"inputs":[{"prevTxId":"a477af6b2667c29670467e4e0728b685ee07b240235771862318e29ddbe58458","outputIndex":0,"sequenceNumber":4294967295,"script":"OP_1","output":{"hellars":1020000,"script":"OP_1 OP_ADD OP_2 OP_EQUAL"}}],"outputs":[{"hellars":1010000,"script":"OP_DUP OP_HASH160 20 0x7821c0a3768aa9d1a37e16cf76002aef5373f1a8 OP_EQUALVERIFY OP_CHECKSIG"}],"nLockTime":0}';
